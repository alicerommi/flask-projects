...
def home():
    if request.method == 'GET':
        ...
        try:
            results2 = engine.execute("""SELECT * FROM "tide_work" where customs_release_status !="CLEARED" or line_release_status !='PAID' or
...
def save_container_form():
    ...
    if ('\n' in container):
        ...
        for i in container:
            i=i.strip()
            if (len(i) < 13):
                sql = "SELECT * FROM 'tide_work' where reference_no == ?"
    ...
    if(len(no_exist)>0):
        ...
        for i in no_exist:
            sql = """insert into tide_work(reference_no,search_status,available,size,customs_release_status,
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                sql = "UPDATE tide_work set reference_no=? where reference_no='" + reference + "'"
...
def delete_ref(val):
    ...
    if request.method == 'POST':
        try:
            print(val)
            sql = "DELETE FROM tide_work WHERE reference_no='" + val + "'"
            engine.execute(sql)
